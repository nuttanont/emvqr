...
const getDataObjectNameSubDataBillpay = models.getDataObjectNameSubDataBillpay;
const getDataObjectNameSubDataCreditTrans = models.getDataObjectNameSubDataCreditTrans;
...
function decode(emvString) {
    ...
    while (inputText.length > 0) {
        ...
        if(emvItem.id == 29){
            emvItem.data = decodeSubDataCreditTrans(emvItem.data)
        }
        if(emvItem.id == 30){
            emvItem.data = decodeSubDataBillpay(emvItem.data)
        }
...
function decodeSubDataBillpay(emvString) {
    const emvObject = {};
    ...
    while (inputText.length > 0) {
        debug.log('inputText', inputText);
        let { emvItem, remainingText } = readNextSubDataBillpay(inputText);
        emvObject[emvItem.id] = emvItem;
        inputText = remainingText;
    }
...
function decodeSubDataCreditTrans(emvString) {
    const emvObject = {};
    ...
    while (inputText.length > 0) {
        debug.log('inputText', inputText);
        let { emvItem, remainingText } = readNextSubDataCreditTrans(inputText);
        emvObject[emvItem.id] = emvItem;
        inputText = remainingText;
    }
...
function readNextSubDataBillpay(inputText) {
    const id = inputText.substring(0, 2);
    const len = checkNumberType(parseInt(inputText.substring(2, 4)));
    const data = inputText.substring(4, len + 4);
    const emvItem = {
        id,
        name: getDataObjectNameSubDataBillpay(id),
        len,
        data
    };
    const remainingText = inputText.substring(len + 4);
...
function readNextSubDataCreditTrans(inputText) {
    const id = inputText.substring(0, 2);
    const len = checkNumberType(parseInt(inputText.substring(2, 4)));
    const data = inputText.substring(4, len + 4);
    const emvItem = {
        id,
        name: getDataObjectNameSubDataCreditTrans(id),
        len,
        data
    };
    const remainingText = inputText.substring(len + 4);
