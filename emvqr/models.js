...
const dataObjectsSchema = {
    ...
    '30': {
        name: 'Reserved for PromptPay - Bill Payment',
        presence: 'M',
    },
    '29': {
        name: 'Credit Transfer with PromptPayID',
        presence: 'M',
    },
    '02-51': {
...
const dataObjectsSchemaSubBillpay = {
    '00': {
        name: 'AID',
        presence: 'M',
    },
...
const dataObjectsSchemaSubCreditTrans = {
    '00': {
        name: 'AID',
        presence: 'M',
    },
...
function getDataObject(stringId) {
    ...
    if (!dataObject) {
        ...
        (id == 29) && (dataObject = dataObjectsSchema['29']);
        (id == 30) && (dataObject = dataObjectsSchema['30']);
...
function getDataObjectSubDataBillpay(stringId) {
    let dataObject = dataObjectsSchemaSubBillpay[stringId];
    if (!dataObject) {

    }
    return dataObject;
}
function getDataObjectSubDataCreditTrans(stringId) {
    let dataObject = dataObjectsSchemaSubCreditTrans[stringId];
    if (!dataObject) {
...
function getDataObjectNameSubDataBillpay(stringId) {
    const dataObject = getDataObjectSubDataBillpay(stringId);
    return dataObject ? dataObject.name : undefined;
}
function getDataObjectNameSubDataCreditTrans(stringId) {
    const dataObject = getDataObjectSubDataCreditTrans(stringId);
    return dataObject ? dataObject.name : undefined;
}
...
module.exports = {
    ...
    getDataObjectNameSubDataBillpay,
    getDataObjectNameSubDataCreditTrans
};